...
function operationsPayload() {
  return {
    ...
    runtime: { schema_version: "runtime-operations.v1", runtime: { status: "ready", sdk: "deepseek-harness-sdk==0.1.2rc1", process_ownership: "one-per-active-session" }, sessions: { active: 1, active_prompts: 0, status_counts: { idle: 1 } }, usage: { input_tokens: 100, output_tokens: 20, cache_read_tokens: 30, cache_write_tokens: 0, reasoning_tokens: 10, model_calls: 1, total_tokens: 150, scope: "adapter_process_lifetime", source: "normalized_dsh_token_usage" }, raw_dsh_events: false },
...
test("operations page renders safe status projection", async ({ page }) => {
  ...
  await expect(page.getByText("deepseek-harness-sdk==0.1.2rc1")).toBeVisible();
});
