...
export function readPaperSubmission(scope:string):PaperCommand|null {
  ...
  if(raw.length > 128*1024) throw Error('原模拟账户操作记录超出范围');
  const value = JSON.parse(raw);
  if(!value || typeof value.key !== 'string' || !/^[A-Za-z0-9_-]{8,96}$/.test(value.key)
    || !['create','order','settlement','controls','rebind','delete'].includes(value.operation)
    || !value.payload || typeof value.payload !== 'object' || Array.isArray(value.payload)
    || (value.operation !== 'create' && !/^paper_account_[0-9a-f]{32}$/.test(value.account_id ?? ''))) throw Error('原模拟账户操作记录无法读取');
  return value;
}
export function beginPaperSubmission(scope:string,input:Omit<PaperCommand,'key'>):PaperCommand {
  ...
  if(raw.length > 128*1024) throw Error('模拟账户操作超出范围');
