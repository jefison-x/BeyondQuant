    ...
    >
      ...
      <main v-if="selected" class="workspace">
        ...
        <el-card shadow="never" class="detail-card"><el-tabs v-model="activeTab">
          <el-tab-pane label="总览" name="overview"><div class="overview-grid"><div class="info-panel"><span>账户版本</span><strong>v{{ selected.version }}</strong></div><div class="info-panel"><span>最后结算日</span><strong>{{ selected.last_settlement_date ?? "尚未结算" }}</strong></div><div class="info-panel"><span>持仓 / 订单 / 成交</span><strong>{{ positions.length }} / {{ orders.length }} / {{ fills.length }}</strong></div><div class="info-panel"><span>风险状态</span><strong>{{ controls?.kill_switch_engaged ? "已暂停新订单" : "允许委托" }}</strong></div></div><div class="account-danger-zone"><span><strong>删除模拟账户</strong><small>从工作区移除账户，但保留不可变成交、流水和审计记录。</small></span><el-button type="danger" plain :loading="busy === 'delete'" @click="deleteAccount">删除账户</el-button></div></el-tab-pane>
