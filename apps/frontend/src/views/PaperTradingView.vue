...
const accountPages = useFilteredPagination(computed(() => accounts.value), (item) => `${item.name} ${item.account_id}`, 12);
const positionPages = useFilteredPagination(computed(() => positions.value), (item) => `${item.symbol ?? ""}`, 30);
const orderPages = useFilteredPagination(computed(() => orders.value), (item) => `${item.symbol ?? ""} ${item.trade_date ?? ""} ${item.status ?? ""}`, 30);
const ledgerPages = useFilteredPagination(computed(() => ledger.value), (item) => `${item.symbol ?? ""} ${item.entry_type ?? ""} ${item.created_at ?? ""}`, 30);
const snapshotPages = useFilteredPagination(computed(() => snapshots.value), (item) => `${item.trade_date ?? ""} ${item.snapshot_id ?? ""}`, 30);
...
async function deleteAccount() {
  ...
  await ElMessageBox.confirm(
    `删除模拟账户“${selected.value.name}”？账户将从工作区移除，历史成交、流水和审计记录仍会保留。`,
    "删除模拟账户",
  ...
  } catch (exc) { error.value = exc instanceof Error ? exc.message : "删除账户失败"; }
  finally { busy.value = ""; }
}
  ...
  <section class="paper-page">
    ...
    >
      ...
      <el-card shadow="never" class="account-rail">
        ...
        <ListFilterPagination v-model:query="accountPages.query.value" v-model:page="accountPages.page.value" :page-size="accountPages.pageSize.value" :total="accountPages.total.value" placeholder="筛选账户名称或编号" label="模拟账户分页">
          <button v-for="account in accountPages.pageItems.value" :key="account.account_id" class="account-item" :class="{ active: selected?.account_id === account.account_id }" @click="selectAccount(account)"><span><strong>{{ account.name }}</strong><small>{{ account.account_id }}</small></span><span class="account-value">¥ {{ money(account.cash) }}</span></button>
        </ListFilterPagination>
        <el-empty v-if="!accounts.length" description="暂无模拟账户" :image-size="72" /><el-divider />
        <div class="create-grid"><el-input v-model="accountName" data-testid="paper-account-name" placeholder="账户名称" /><label class="initial-cash-field"><span>初始资金</span><el-input-number v-model="initialCash" data-testid="paper-initial-cash" :min="1000" :step="10000" controls-position="right"><template #prefix>¥</template></el-input-number></label><el-button data-testid="paper-create-account" type="primary" :loading="busy === 'create'" @click="createAccount">新建账户</el-button></div>
      </el-card>
      </template>
      ...
      <main v-if="selected" class="workspace">
        ...
        <el-card shadow="never" class="detail-card"><el-tabs v-model="activeTab">
          <el-tab-pane label="总览" name="overview" lazy><div class="overview-grid"><div class="info-panel"><span>账户版本</span><strong>v{{ selected.version }}</strong></div><div class="info-panel"><span>最后结算日</span><strong>{{ selected.last_settlement_date ?? "尚未结算" }}</strong></div><div class="info-panel"><span>持仓 / 订单 / 成交</span><strong>{{ positions.length }} / {{ orders.length }} / {{ fills.length }}</strong></div><div class="info-panel"><span>风险状态</span><strong>{{ controls?.kill_switch_engaged ? "已暂停新订单" : "允许委托" }}</strong></div></div><div class="account-danger-zone"><span><strong>删除模拟账户</strong><small>从工作区移除账户，但保留不可变成交、流水和审计记录。</small></span><el-button type="danger" plain :loading="busy === 'delete'" @click="deleteAccount">删除账户</el-button></div></el-tab-pane>
          <el-tab-pane label="持仓" name="positions" lazy><div class="tab-toolbar"><span class="muted">总数量、可卖数量与当日锁定数量分开记录</span><el-button type="primary" plain :disabled="!positions.length" @click="openSettlement">日终结算</el-button></div><ListFilterPagination v-model:query="positionPages.query.value" v-model:page="positionPages.page.value" :page-size="positionPages.pageSize.value" :total="positionPages.total.value" placeholder="筛选股票代码" label="持仓分页"><el-table :data="positionPages.pageItems.value" empty-text="暂无持仓"><el-table-column prop="symbol" label="标的" min-width="130" /><el-table-column prop="quantity" label="总数量" min-width="95" /><el-table-column prop="sellable_quantity" label="可卖" min-width="95" /><el-table-column prop="locked_quantity" label="T+1 锁定" min-width="105" /><el-table-column label="平均成本" min-width="110"><template #default="{ row }">¥ {{ money(row.average_cost) }}</template></el-table-column><el-table-column label="最新标记" min-width="110"><template #default="{ row }">{{ row.market_price == null ? '-' : `¥ ${money(row.market_price)}` }}</template></el-table-column></el-table></ListFilterPagination></el-tab-pane>
          <el-tab-pane label="订单与成交" name="orders" lazy><ListFilterPagination v-model:query="orderPages.query.value" v-model:page="orderPages.page.value" :page-size="orderPages.pageSize.value" :total="orderPages.total.value" placeholder="筛选代码、日期或状态" label="订单分页"><el-table :data="orderPages.pageItems.value" empty-text="暂无订单" @row-click="showOrder"><el-table-column prop="trade_date" label="交易日" min-width="105" /><el-table-column prop="symbol" label="标的" min-width="125" /><el-table-column label="方向" min-width="80"><template #default="{ row }">{{ row.side === 'buy' ? '买入' : '卖出' }}</template></el-table-column><el-table-column prop="quantity" label="数量" min-width="90" /><el-table-column label="价格" min-width="100"><template #default="{ row }">¥ {{ money(row.price) }}</template></el-table-column><el-table-column label="状态" min-width="100"><template #default="{ row }"><el-tag :type="row.status === 'filled' ? 'success' : 'warning'">{{ statusLabel(row.status) }}</el-tag></template></el-table-column><el-table-column label="结果" min-width="170"><template #default="{ row }">{{ row.blocked_reason ? reasonLabel(row.blocked_reason) : `费用 ¥ ${money(Number(row.fees ?? 0) + Number(row.tax ?? 0))}` }}</template></el-table-column></el-table></ListFilterPagination></el-tab-pane>
          <el-tab-pane label="资金流水" name="ledger" lazy><ListFilterPagination v-model:query="ledgerPages.query.value" v-model:page="ledgerPages.page.value" :page-size="ledgerPages.pageSize.value" :total="ledgerPages.total.value" placeholder="筛选代码、类型或时间" label="资金流水分页"><el-table :data="ledgerPages.pageItems.value" empty-text="暂无资金流水"><el-table-column prop="created_at" label="时间" min-width="180" /><el-table-column prop="entry_type" label="类型" min-width="120" /><el-table-column prop="symbol" label="标的" min-width="120" /><el-table-column label="现金变动" min-width="130" align="right"><template #default="{ row }"><span :class="{ positive: Number(row.cash_delta) > 0, negative: Number(row.cash_delta) < 0 }">¥ {{ money(row.cash_delta) }}</span></template></el-table-column><el-table-column label="费用" min-width="100" align="right"><template #default="{ row }">¥ {{ money(row.fees) }}</template></el-table-column><el-table-column prop="snapshot_id" label="关联快照" min-width="220" show-overflow-tooltip /></el-table></ListFilterPagination></el-tab-pane>
          <el-tab-pane label="结算快照" name="snapshots" lazy><div class="tab-toolbar"><span class="muted">每日快照不可改写；估值变动不伪装成现金流水</span><el-button type="primary" plain :disabled="!positions.length" @click="openSettlement">执行结算</el-button></div><ListFilterPagination v-model:query="snapshotPages.query.value" v-model:page="snapshotPages.page.value" :page-size="snapshotPages.pageSize.value" :total="snapshotPages.total.value" placeholder="筛选交易日或快照" label="结算快照分页"><el-table :data="snapshotPages.pageItems.value" empty-text="暂无结算快照"><el-table-column prop="trade_date" label="交易日" min-width="110" /><el-table-column label="权益" min-width="120"><template #default="{ row }">¥ {{ money(row.equity) }}</template></el-table-column><el-table-column label="市值" min-width="120"><template #default="{ row }">¥ {{ money(row.market_value) }}</template></el-table-column><el-table-column label="当日盈亏" min-width="120"><template #default="{ row }">¥ {{ money(row.daily_pnl) }}</template></el-table-column><el-table-column label="当日收益" min-width="110"><template #default="{ row }">{{ row.daily_return == null ? '-' : `${(Number(row.daily_return) * 100).toFixed(2)}%` }}</template></el-table-column><el-table-column prop="snapshot_fingerprint" label="快照指纹" min-width="220" show-overflow-tooltip /></el-table></ListFilterPagination></el-tab-pane>
          <el-tab-pane label="风险与迁移" name="risk" lazy><div class="risk-grid"><section class="risk-panel"><h3>显式风险控制</h3><el-form label-position="top"><el-form-item label="暂停全部新订单"><el-switch v-model="killSwitch" /></el-form-item><el-form-item label="开关原因"><el-input v-model="killReason" maxlength="256" show-word-limit /></el-form-item><el-form-item label="单笔最大金额（CNY）"><el-input-number v-model="maxOrderNotional" :min="100" :step="10000" controls-position="right" /></el-form-item><el-button type="primary" :loading="busy === 'controls'" @click="saveControls">保存风险控制</el-button></el-form></section><section class="risk-panel"><h3>冻结股票池</h3><p class="muted">仅空仓账户可显式重绑；股票池编辑不会改变既有账户。</p><el-select v-model="poolId" placeholder="选择股票池" class="wide-select"><el-option v-for="pool in pools" :key="String(pool.pool_id)" :label="String(pool.name)" :value="String(pool.pool_id)" /></el-select><el-button :disabled="positions.length > 0" :loading="busy === 'binding'" @click="rebind">重新绑定当前快照</el-button></section><section class="risk-panel"><h3>BYQ 账户资产包</h3><p class="muted">导入会校验摘要与引用，并生成新账户 ID；不会覆盖现有账户或导入所有权。</p><div class="button-row"><el-button :loading="busy === 'export'" @click="downloadBundle">导出 JSON</el-button><el-button :loading="busy === 'import'" @click="importInput?.click()">导入为新账户</el-button></div></section></div></el-tab-pane>
        </el-tabs></el-card>
      </main>
