...
const credentialForm = reactive({ label: "", secret: "" });
const profileForm = reactive({ credential_id: "", key_name: "", display_name: "", model: "deepseek-v4-flash", temperature: 0.2, reasoning_enabled: false });
...
const selectedModel = computed(() => settings.value?.models.find((item) => item.model === profileForm.model));
...
function openCredential(item: ModelCredential | null = null) {
  ...
  credentialForm.label = item?.label ?? "个人 DeepSeek API";
...
async function saveCredential() {
  ...
  try {
    if (editingCredential.value) {
      ...
      await createModelCredential({ purpose: "model_api_key", provider: "deepseek", scope: "user", label: credentialForm.label, secret: credentialForm.secret, idempotency_key: `browser-create-${Date.now()}` });
...
function openProfile() {
  profileForm.credential_id = activeCredentials.value[0]?.credential_id ?? "";
  profileForm.key_name = "";
  profileForm.display_name = "";
  profileForm.model = settings.value?.models[0]?.model ?? "deepseek-v4-flash";
...
async function saveProfile() {
  ...
  busy.value = true;
  try {
    await createModelProfile({ ...profileForm, provider: "deepseek" });
...
<template>
  <section class="my-space-page">
    ...
    <el-dialog v-model="credentialDialog" :title="editingCredential ? '替换模型凭据' : '添加模型凭据'" width="min(520px, 92vw)" destroy-on-close>
      <el-form label-position="top"><el-form-item label="名称"><el-input v-model="credentialForm.label" maxlength="120" /></el-form-item><el-form-item label="DeepSeek API Key"><el-input v-model="credentialForm.secret" type="password" show-password autocomplete="new-password" placeholder="写入后仅显示掩码" /></el-form-item></el-form>
    ...
    <el-dialog v-model="profileDialog" title="新建模型档案" width="min(560px, 92vw)" destroy-on-close>
      <el-form label-position="top"><el-form-item label="档案名称"><el-input v-model="profileForm.display_name" /></el-form-item><el-form-item label="唯一键"><el-input v-model="profileForm.key_name" placeholder="research-fast" /></el-form-item><el-form-item label="凭据"><el-select v-model="profileForm.credential_id" class="full"><el-option v-for="item in activeCredentials" :key="item.credential_id" :label="`${item.label} · ${item.masked}`" :value="item.credential_id" /></el-select></el-form-item><el-form-item label="模型"><el-select v-model="profileForm.model" class="full"><el-option v-for="model in settings?.models" :key="model.model" :label="model.display_name" :value="model.model" /></el-select></el-form-item><el-form-item label="温度"><el-slider v-model="profileForm.temperature" :min="0" :max="2" :step="0.1" show-input /></el-form-item><el-form-item label="推理模式"><el-switch v-model="profileForm.reasoning_enabled" :disabled="!selectedModel?.reasoning_supported" /></el-form-item></el-form>
