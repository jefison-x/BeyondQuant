...
**Phase 79**：可靠 LightGBM 闭环已完整接入小巴；小巴可查询封闭模型能力、定位研究对象、创建策略、
管理可信训练与样本外预测，并将冻结信号交给既有 `backtest-task.v1` 复核。策略批准及预测、回测等
有后果动作仍由用户分别确认，DSH 不接收模型对象、raw features、raw predictions 或 raw signals。
