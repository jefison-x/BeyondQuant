...
def normalize_web_research_evidence(value: object) -> dict[str, object]:
    """Convert the Agent-facing candidate shape into the persisted v1 shape.

    Source identifiers are a BYQ storage concern.  The model supplies zero-based
    source indexes for claims; BYQ derives stable identifiers from canonical
    public URLs and never asks the model to satisfy an internal identifier regex.
    """

    content = copy.deepcopy(_object(value, "content"))
    ...
    for index, raw in enumerate(raw_sources):
        ...
        if "source_id" in source:
            raise ValueError("source_id is generated by BYQ and must not be supplied")
        url = _url(source.get("url"))
        source_id = f"source_{hashlib.sha256(url.encode('utf-8')).hexdigest()[:20]}"
    ...
    for index, raw in enumerate(raw_claims):
        ...
        if "source_ids" in claim:
            raise ValueError("source_ids are generated by BYQ and must not be supplied")
        indexes = claim.pop("source_indexes", None)
        if (
            ...
            or any(
                ...
                or item < 0
                or item >= len(source_ids)
                for item in indexes
            )
        ):
            raise ValueError("claim source_indexes are invalid")
        claim["source_ids"] = [source_ids[item] for item in indexes]

    return validate_web_research_evidence(content)
