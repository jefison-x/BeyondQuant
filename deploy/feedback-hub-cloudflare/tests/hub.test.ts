...
describe("isolated Cloudflare GitHub publisher", () => {
  ...
  it.each(["empty", "permit_denied", "later_match", "exhausted", "later_error", "cross_page_conflict"])("reconciles bounded catalog before publication: %s", async (scenario) => {
    const value = await envelope();
    const event: PublicationEvent = {
      event_id: `feedback_outbox_${hexId()}`,
      feedback_id: `central_feedback_${hexId()}`,
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
      ...
      if (url.includes("?state=all")) {
        ...
        if (scenario === "empty" || scenario === "permit_denied") return new Response("[]", { status: 200 });
    ...
    const pages = ["empty", "permit_denied"].includes(scenario) ? 1 : scenario === "exhausted" ? 5 : 2;
