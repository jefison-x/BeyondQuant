...
**Named D15 superseding assessment status (2026-09-22): established, one isolated worktree/Draft PR** (`codex/v090-d15-superseding-assessment`, base dynamic `origin/main` `8a4e4fe`; evidence `docs/evidence/d15/d15-superseding/`). The fail-able, machine-readable assessment (`scripts/d15/superseding_assessment/{contract.v1.json,observer.py,build_provenance.py}`) independently derives, from raw evidence, the current overlay and the ADR-0084-consistent truth: historical D15-G `NO_GO_PRESERVED` (not rewritten), the replacement gate **BYQ session failure containment and business recovery** `PASS` (real isolated acceptance 9/9 plus containment/classification, cleanup zero and production untouched), the coherent DSH `0.1.5-rc.1` repository default upgrade `PASS`, B1/B2 `BLOCKED_EXTERNAL`, B3/B4 `PASS_CANDIDATE`, and native independent child resume `NOT_IMPLEMENTED`. For the **actually adopted scope** it derives candidate compatibility `PASS` (B1/B2 do not gate it per ADR-0084 §3) and promotion `REPO_DEFAULT_PROMOTED` (repository default only; production deployment/release/tag remain separate and are not claimed). It bounds the permissible R3 scope to `[safe_failure, observation, cleanup, new_generation_recovery]` and keeps `R3_RESUME = NO`. The observer rejects overclaims, rewrites of the historical verdict, R3 full resume, widened/dropped R3 scope, missing/hash-mismatched evidence and self-declared verdict fields; `--selfcheck` rejects 21 controls (19 defect-targeting) with a known-good fixture that establishes. Because the assessment adds build inputs (`scripts/`, `tests/`, `services/runtime-adapter`), the build identity advances `dsh-0.1.5rc1-post-u8.200 -> post-u8.201`; `scripts/dsh/build_revision.py`, `Dockerfile.post-u8-candidate` and `scripts/v090/dsh_default_upgrade/verify.py` are synced, and the `default-upgrade.v1.json` digest entries for the changed build-identity files (build revision selector, production Dockerfile, promoted release descriptor and generated identity) are refreshed (digest-only, causally required) while the `.200` manifest and all historical D15 evidence are preserved. Boundaries unchanged: B1/B2 stay `BLOCKED_EXTERNAL`, historical D15-4/D15-5/D15-G verdicts are not rewritten, `R3_RESUME = NO`, the production selector/default is not switched beyond the already-executed repository default, and no deploy/release/tag, Phase 100 resume or 0.10 start occurs. **This PR does not execute the final 0.9 closeout**; the retained closeout order becomes: gate -> real acceptance -> coherent default upgrade -> **named D15 superseding assessment (delivered by this PR)** -> **0.9 closeout (separate, maintainer-gated)**.
...
P4-B 已合并；P4-C1 为当前候选事实（Draft PR，仅其自身 scoped verdict）。P4-C2/P4-D 尚未开始；P4 整体证据仍保持 `all_pass=false`。P4-C1 不声称 DSH 原生跨进程恢复，也不解锁或宣称 P4-C2/P4-D。
