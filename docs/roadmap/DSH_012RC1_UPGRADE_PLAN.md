...
2026-09-07 维护者处置：真实业务发现阻断缺陷，明确要求停止观察并优先修复。
U8 按 `CLOSED_EARLY_REMEDIATION_REQUIRED` 收尾，保留4小时25分钟/78次采样和未通过结论；
不将24小时门禁或T40改为PASS。此决定允许收尾合并后进入已授权的Post-U8修复，
不回退DSH，暂缓数据中心扩容。修复后以新构建重新验收观察。以下为原始正常验收标准。
