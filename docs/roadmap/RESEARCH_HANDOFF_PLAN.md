...
| H2 持久任务交接 | F3/F4/F5/F10 | 基于任务、审批、job 和投递事实区分完成、待审批、待作业、缺许可、已排队、阻塞；原目标及对象不靠聊天摘要猜测 | 待实现。回合终止而无后续执行者可被识别；重复/迟到事件、错误归属及重启不能制造 running/completed；Product API 与真实浏览器一致 |
| H3 授权续接闭环 | 复用已交付 F6，补 F4/F5 连接 | 在已确认任务及许可下将交接接入现有投递、预算、取消与回执机制；不新增通用调度器 | 待实现。无许可不启动；重复事件不重跑；撤销/过期/额度不足拒绝；未知提交先核对；审批完成不自动授权新敏感动作 |
