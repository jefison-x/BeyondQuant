...
5. D15 pre-gate 修复切片：
   ...
   - `subagent-byq-adapter-restart`（owner `d15-4-candidate-composition-hookup`）；
   - `terminal-adapter-restart` / `terminal-dsh-runtime-restart`（owner `d15-5-candidate-attachment-layer`，非 post-GO R4）。
