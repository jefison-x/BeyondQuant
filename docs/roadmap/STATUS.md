...
未声明/0/bool/负/冲突/超 profile 的 output cap 在请求前 fail closed；记录 provider 实际 usage 与 declared 上限分离，
**无法证明实际 output usage 时 fail closed（`actual_usage_unknown`，不转发/提交）**；非跨进程固定 2 次。
