...
- 维护执行：[DSH 0.1.2rc1 升级方案](DSH_012RC1_UPGRADE_PLAN.md) 的 U0 已 `VERIFIED`，维护者于
  ...
  当前执行 U6 发布与回滚演练：最小排空门禁、跨版本审批续接、逻辑备份/实际恢复及隔离 old→new→old 诊断通过；
  维护者已明确授权保留历史 release 描述、报告和失败测试不变，新增 U6 独立构建身份并重新认证；
  ADR-0061 已记录该决定，历史身份检查及171项架构测试通过；独立构建复验和最终模型/浏览器演练正在执行；
  T38/T39 尚未完成，T40 留给 U7-U8。维护者已授权 U1–U8 串行开发、自动 push/Draft PR，并在 CI-green 后按
