...
| 0.9.1 | ADR-0085 会话/长研究稳定性整改：确定性研究计划、有界 Agent 数据面、统一接续状态与真实复合研究闭环 | P0→P4 逐 PR 合并；真实三轮研究和故障矩阵通过；正式发布清单、部署与 tag/release 独立授权 |

> **2026-09-22 0.9 开发收口状态**：0.9 的 **开发收口**（development closeout）已完成并通过机器可读、
> 可失败的最终矩阵验收（`V090_DEVELOPMENT_CLOSEOUT_COMPLETE`，证据
> [`docs/evidence/v090-final-closeout/`](../evidence/v090-final-closeout/README.md)）。上表 0.9.x 门槛
> 达成：F2 由当前 `complete=true` 的逐接口台账覆盖；剩余全接口审计 `discovered=reviewed=verified=569`；
> 复合研究故障回归以 `PASS_AS_SCOPED` 记录（旅程 PASS、13 个 gating 行 PASS、4 个非 gating 行诚实
> BLOCKED）。**开发收口不等于正式 0.9.0 发布**：0.9.0 行的发布清单门槛仍为 `OPEN_NOT_ATTEMPTED`，
> 与生产部署、正式 tag/release 一样是独立的维护者决定。仓库默认 DSH `0.1.5-rc.1` 不是生产部署。
> 下一状态为维护者测试与 0.9.x 小版本功能追加/优化窗口；**0.10 与 Phase 100 恢复仍未授权**。

> **2026-09-22 0.9.1 决定**：维护者将 ADR-0085 P0–P4 明确定为 0.9.1 稳定性版本。0.9.1
> 先完成开发与真实闭环验收，再单独决定版本写入、发布清单、生产部署和 tag/release；该决定不把
> 0.9.0 的历史开发收口或尚未执行的正式发布门禁改写为已发布，也不授权启动 0.10.0。
...
> 2026-09-20 审计说明：0.9.0/0.9.x 门槛映射、历史证据结论与缺口台账见
> [`docs/evidence/v090-closeout/`](../evidence/v090-closeout/README.md)。历史 `v0.9.0-beta`
> tag 只是预发布标记，**不**等于正式 0.9.0 closeout；正式发布清单须绑定源码 SHA、全部服务
> digest 与实际服务组合、迁移分类、备份、回退版本与独立发布验收。审计仅记录缺口，不创建或
> 移动任何 tag/release。
>
> **版本范围边界**：S3 / 历史成分准备属于 **0.10.0**（见上表 0.10.0 行），**不属于** 0.9.x
> 门槛。历史 Post-U8 S3 调查保持 `blocked` 事实，但在 ADR-0068/AGENTS 规则 21 下缺失 Community
> 来源不是开发阻塞；它移交冻结的 Phase 100，用 Tushare P100-C/P100-D 时点证据解决，不申请
> Community 豁免、不用当前关系回填。0.9 closeout 顺序不以 S3 开头，也不被 S3 阻塞。
