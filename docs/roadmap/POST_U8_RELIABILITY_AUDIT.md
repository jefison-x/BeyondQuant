...
[ADR-0066（Proposed）](../architecture/adr/ADR-0066-domain-validation-call-admission.md)，
请求明确接受内部调用凭证及领域纠错准入边界。未改运行时、数据库、MCP 或生产行为；
没有将该方案标为 Accepted 或把现有角色授权描述成已证实的生产越权。
既有 `F7-NEXT-SLICE.md` 的 AgentRun 引用方案须以本 ADR 的精确调用证明补充，不能仅靠引用实现准入。

本次为文档/决策切片：5 份文档检查通过、214 项架构与共享合同检查通过；旧 .5 构建清单仍匹配。
没有新增运行时认证结论，没有运行 Backend 全套、浏览器或付费模型测试。提案待接受，未推送、未合并、未部署。
...
## F7 有效成果原子回执补测（2026-09-08）

普通/ML 策略均补齐 Backend 取消先到、提交后 ASGI 回复丢失、提交中真实数据库锁竞争
回归，共新增 6 项；相关五个测试文件 69 PASS、交付规则 12 PASS。
已有实现保持单成果/单回执与根取消终态，本批无业务实现修改。
新增独立 `.14` 源身份，历史 manifests 不变；不是新镜像认证或跨服务矩阵全通过。
完整限制及结果见[原子回执证据](../evidence/post-u8-r1/F7-ATOMIC-RECEIPT.md)。
下一步仍需实际 MCP/ML 迟到取消、跨服务未知回执和全研究流程审计；F7/F6/S3 不关闭。

## F7 最终核对完成（2026-09-08，后续结论）

前述 F7 待补项已完成具名验证，并发现、修复普通/ML Agent 回执丢失安全字段诊断的问题。
`.15` 实际链路验证错误提示、一次修正成功、二次失败原生停止及合法请求撤销；
`.14` 真实 MCP 断连/唯一回执和 ML 迟到取消记录保留，历史失败未改记通过。
最终本地26/26、独立清理核验通过；[最终矩阵](../evidence/post-u8-r1/F7-ACCEPTANCE-MATRIX.md)
明确范围与证据边界。F7 首批两工具关闭，F6/S3 及全研究流程/接口审计不随之关闭。
维护者明确要求 F7 全部完成后推送合并；当前进入该条件性交付门禁，不部署生产。

## 2026-09-10 F6 后台续接功能验收完成

基于 Accepted ADR-0062/0065 和维护者“F6 完成前不要停止，所有授权都允许”的当前任务授权，
完成原任务持久通知、显式许可、累计保守 token 额度、MCP 原任务范围准入和持久结算。
`.29` 全量本地 CI 29/29、真实 Product API 桌面/手机、三回合领域链路及 Gateway 重启通过；
固定合成目标的真实模型另外在一个后台回合内完成预测、信号、原生回测与 validated 对比报告，
精确 `.29` 镜像的单请求付费预算关键点通过。先前失败、停止的资格运行及探针错误均保留。
...
## 2026-09-12 审批续接与工具合同故障修复

维护者在只读会话诊断后授权修复。新增动作/资源类型入口校验，旧错误审批续接前阻断，
精确关联任务的审批阻塞投影，研究参数与常见回测422指导，以及 Gateway 核对失败持久退避。
[本次证据与限制](../evidence/session-reliability-20260912/AUDIT.md)单独记录验证和交付状态。
不延长根回合上限，不扩大 F7 首批工具范围，不将历史提交回执改记成功或自动续跑生产任务。
