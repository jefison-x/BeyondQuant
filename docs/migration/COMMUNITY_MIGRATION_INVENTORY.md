...
The read-only Community `StrategyView.vue`, `BacktestView.vue`, Operations
`ModelOperationsView.vue`, `DataSourceConfig.vue`, and `DataSync.vue` were
inspected before optimizing the BYQ Strategy, Model Research, Backtest, and
Data Management loading paths.
...
| Data Sync and Data Source Config load bounded catalogue/configuration data first, then progress after explicit action | Operational status should not materialize immutable ML training inputs that the page never displays. | `PORT_UX` / `REFACTOR` | Keep BYQ's combined audited Data Center, but project only public summary columns for recent ML task status; do not copy fake/TODO or direct-provider paths. |
