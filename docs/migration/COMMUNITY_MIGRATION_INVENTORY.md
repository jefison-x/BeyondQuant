...
| Backtest catalogue pagination, request sequencing, detail cache and lazy result tabs | Hidden result tables should not mount before the user opens them; filtering/pagination must bound transport as well as rendering, and stale responses must not replace a newer selection. | `PORT_UX` / `REFACTOR` | Reuse BYQ immutable Backtest results through browser-only summary/page projections, server-side filter/page reads and lazy bounded evidence tabs. Preserve the full result and Agent MCP analysis contracts; do not copy Community engine/API/runtime code. |
...
| Strategy list/detail confirmation-based delete affordance; no dedicated Community ML research delete contract | Users need a visible, confirmed way to discard an unexecuted study, while execution evidence must remain auditable. | `PORT_UX` / `REPLACE` | Reuse only the confirmation affordance. BYQ Product API atomically soft-supersedes a never-executed ML strategy and its approvals; any training, prediction, or Backtest history blocks deletion. Community direct API and physical-delete semantics are not reused. |
