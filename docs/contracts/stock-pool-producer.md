...
- 创建可明确选择 `tracking_mode=historical_snapshot` 或 `follow_index`（兼容默认）。历史模式
  ...
  历史模式必须提交明确且不晚于上海当前日的日期；遗漏或空日期不能默认成今天。
  持续跟踪模式的默认日期使用 Asia/Shanghai，与目录查询保持一致。

- 既有指数月度入库先核对每行的指数、规范日期和月份；同月事务锁内拒绝丢失已验证日期
  或静默改写同日已验证内容。空/部分响应和内容冲突不构成删除或覆盖授权，旧数据保持不变。
  相同内容重放和保留已有日期的新增快照仍允许；这不是 Community bulk-import 资格。
