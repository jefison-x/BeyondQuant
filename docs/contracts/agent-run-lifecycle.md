...
ADR-0064 已实现原本地卷、同次 Linux 启动下的 Adapter 崩溃证据恢复：持久化原提交摘要、
精确 root/generation 和事件高水位，通过原锁 nonce/device/inode/boot_id 及排他锁证明
原执行者失效后生成一次 interrupted 证据，走同一 Gateway→Backend 原子收尾路径。
不构建 Harness、不重投提示，不改变独立业务任务。原提示幂等查询返回原接受回执。
恢复核查每 30 秒一次，失败预算 8 次/24 小时在请求前持久扣除，耗尽保持 unavailable。
日志上限 8 MiB，写入失败阻止新模型回合；不裁剪原证据。终态投递预算保持不变。

主机重启、复制卷、非本地文件系统、锁身份缺失以及历史无绑定记录仍不可自动恢复；
缺失绑定或回执不得靠同会话最新对象、当前进程下所有历史记录或时间猜测。
收尾是异步最终一致性，不承诺终态发生瞬间Backend即已更新；同进程跨轮复用受上述确认
屏障保护。完全崩溃没有终态证据时仍不能靠404、时间或最新任务猜测清理历史记录。

本合同覆盖有精确规范化注册/终态及上述可证明崩溃的跨服务投递、收尾和回执。不等于所有无终态事故、R4/F10
