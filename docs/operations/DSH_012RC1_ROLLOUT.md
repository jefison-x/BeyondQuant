...
现行常规升级规则：依据 [ADR-0059 的 2026-09-10 修订](../architecture/adr/ADR-0059-development-governance-and-ci-integrity.md)，
升级前备份数据并保留当前配置和精确旧镜像，部署已通过 CI 的新镜像，完成健康和基本业务验证；
失败优先切回旧镜像，保留当前数据库。复用既有部署/回退配置，不每次重做完整数据库恢复演练。
仅数据库结构、存储格式、不兼容或不可逆数据变更，或恢复证据失效时，补充针对性恢复验证。
以下 U7/U8 专项内容保留其历史适用范围，不作为普通更新重新资格认证或24小时观察的通用前置条件。

历史专项方案状态：PLANNED。仅在[主方案](../roadmap/DSH_012RC1_UPGRADE_PLAN.md) U6/U7 前置条件满足后执行。
...
5. 记录字节数、SHA-256、创建时间、备份对象、可读验证与权限；`pg_restore --list` 只算可读，
   只有声称“实际恢复成功”时才需在隔离数据库实际 restore 并验证持久对象数量/关键关系；
   普通升级仅要求备份成功、可读和校验和，不要求每次执行该演练。
