"""Pause a valid synthetic tool before an isolated transport-loss test."""
import json
import re
import threading
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/control":
            ...
            assert re.fullmatch(r"ci-f7-loss-[a-z]+", body["owner"])
        ...
        if self.path == "/release":
            assert body == {"release": True} and state["paused"]
            release.set()
            self.send_response(204)
            self.end_headers()
            return
        ...
        response = stream(wire_name, args, count,
            text="Synthetic transport outcome unknown. No completion is claimed; do not retry." if name is None else None)
        self.send_response(200)
