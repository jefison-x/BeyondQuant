"""Non-forwarding Provider: schema-valid domain rejection then one failed repair."""
import json
import re
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/control":
            ...
            assert re.fullmatch(r"ci-f7-diagnostic-(strategy|ml)", body["owner"])
        ...
        assert count <= 4 and state["total"] <= 8, "native stop did not bound repair"
        ...
        if count == 1:
            ...
            if action == "byq_ml_strategy_create":
                strategy["split"]["train"]["start"] = "2020-02-31" if count == 3 else "2020-02-30"
