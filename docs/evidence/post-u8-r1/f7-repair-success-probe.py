"""Actual official runtime/MCP/Backend safe diagnostic and successful repair."""
import json
import os
import time
import httpx
...
for action, owner, field in (
    ("byq_strategy_validate", "ci-f7-repair-strategy", "strategy.script"),
    ("byq_ml_strategy_create", "ci-f7-repair-ml", "split.train.start"),
):
    with httpx.Client(timeout=10) as client:
        if os.environ.get("VERIFY_ONLY") != "1":
            ...
            assert state["calls"] == 5 and state["hint_observed"] is True, state
            public = client.get(base)
            public.raise_for_status()
            encoded = json.dumps(public.json())
            assert "domain-correction-stopped" not in encoded
            assert "synthetic_private_module" not in encoded
            assert "request_sha256" not in encoded and "input_sha256" not in encoded
            assert any(e["kind"] == "session.result" for e in public.json()["events"])
            print("PUBLIC_RESULT " + session, flush=True)
        roots, claims, artifacts, runs = inspect(owner)
        assert roots == runs == [{"status": "completed"}]
        assert len(claims) == 2 and artifacts == 1
        assert claims[0]["result_json"]["reason"] == "domain_validation_failed"
        assert claims[1]["result_json"]["state"] == "succeeded"
        assert claims[0]["status"] == "correctable_failure" and claims[0]["result_json"]["validation"]["field"] == field
        assert claims[1]["status"] == "succeeded"
        assert "synthetic_private_module" not in json.dumps(claims)
        print("PASS one successful repair " + owner, flush=True)
with httpx.Client(timeout=5) as client:
    assert client.get("http://byq-f7-provider:8801/status").json()["total"] == 10
