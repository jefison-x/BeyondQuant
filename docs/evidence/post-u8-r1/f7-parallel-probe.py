...
for action, owner in (("byq_strategy_validate", "ci-f7-parallel-strategy"), ("byq_ml_strategy_create", "ci-f7-parallel-ml")):
    with httpx.Client(timeout=15) as client:
        ...
        if not existing:
            ...
            client.post("http://backend:8000/v1/users", headers={"x-byq-actor-role": "admin"}, json={
                "username": owner, "display_name": "F7 native synthetic", "password": "ci-f7-parallel-only", "role": "user"}).raise_for_status()
        client.post("http://gateway:8100/api/product/auth/login", json={"username": owner, "password": "ci-f7-parallel-only"}).raise_for_status()
        if existing:
        ...
        assert 7 <= status["calls"] <= 9 and status["root_calls"] == 3
        assert all(2 <= value <= 3 for value in status["children"].values())
        if not existing:
...
with httpx.Client(timeout=5) as client:
    assert 14 <= client.get("http://byq-f7-provider:8801/status").json()["total_calls"] <= 18
