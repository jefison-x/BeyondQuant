...
def call(client, proof, *, root=None, generation=None, changed=False):
    ...
    response = client.post("http://mcp:8300/mcp/v1", headers=headers, json={"jsonrpc": "2.0", "id": 1,
        "method": "tools/call", "params": {"name": "byq_ml_strategy_create", "arguments": {
...
with httpx.Client(timeout=12) as client:
    client.post("http://backend:8000/v1/users", headers={"x-byq-actor-role": "admin"}, json={
        "username": OWNER, "password": "ci-ml-late-http-only", "display_name": "Synthetic late HTTP", "role": "user"}).raise_for_status()
    client.post("http://gateway:8100/api/product/auth/login", json={"username": OWNER, "password": "ci-ml-late-http-only"}).raise_for_status()
    ...
    print(json.dumps({"status": "PASS", "conversation": session, "roots": [r["status"] for r in cancelled[0]],
        ...
        "scope": "scripted ML strategy schema receipt replay and cancel; not all F7 races"}), flush=True)
