...
`.41` 完整本地 CI 为29通过、1失败：普通组件、20次生命周期、smoke和13项真实浏览器通过，
最后 F6 隔离栈仍以 `tests.f6_synthetic_runtime` 启动，漏配测试目录挂载，因生产镜像已移除测试而无法启动。
修复仅在 F6 的隔离 Compose override 加入同样的只读测试挂载，不把测试放回生产镜像。生产未切换。
