...
| [`status.snapshot.txt`](status.snapshot.txt) | exact provenance-bound historical `STATUS.md` bytes used to replay the closeout observer in shallow CI clones; `.txt` prevents current-link lint from resolving its historical relative links in the evidence directory |
