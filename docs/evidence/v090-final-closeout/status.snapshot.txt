...
（已并入 `main`），**ADR-0085 P2 审批与异步事件统一已合并**（已并入 `main`），**ADR-0085 P3 有界研究判断回合已合并**
（已并入 `main`）：框架无关的有界 `research-stage-input.v1`/`research-proposal.v1` 封闭合同、有界 stage input
（排除 raw/full signal snapshot、bars/frame/index 列表并设显式 byte/item 上限）、只读前台有界
`research-judgment-turn` 角色（静态精确只读 allowlist，无写/审批/路由/身份工具）、每 stage 耐久两次模型调用
准入、原子 proposal+progress 提交与首次无 durable progress 即 `needs_attention/no_durable_progress` 栅栏，
且不新增任何通用 plan/event/proposal 写路由；历史 D15 snapshot（D15-4/D15-5/D15-G）不改写。
当前唯一获授权实现切片是 **ADR-0085 P4 真实闭环与故障矩阵**。
...
<!-- byq:harness-continuation=adr-accepted-p0-merged-p1-merged-p2-merged-p3-merged-p4-current-version-0.9.1 -->
...
| 维护（当前） | **0.9 独立最终开发收口已完成**（marker `v090-final-closeout=complete`）；业务恢复真实验收保持 `v090-business-recovery-acceptance=real-passes`，仓库默认 DSH 升级保持 `v090-dsh-default-upgrade=promoted`，具名 D15 assessment 保持 `v090-d15-superseding-assessment=established`；B1/B2 仍 `BLOCKED_EXTERNAL`，`terminal-adapter-restart` 与 `terminal-dsh-runtime-restart` 均保持候选/资格层 **PASS**，历史 D15-G `NO_GO` 与 `R3_RESUME = NO` 不改写。真实复合研究接续事故后，ADR-0085 已于 2026-09-22 获维护者完整接受；审计见 `docs/evidence/harness-continuation-audit-20260922/`。**ADR-0085 P0（止损与事实一致性）已并入 `main`**（有界 signal snapshot MCP 投影、data-ready 精确 backtest task/next action、budget exhaustion→ResearchTask needs_attention、RuntimeGeneration 终态关闭、UI 预算/自动完成语义修正、停用旧 grantless 通用模型 data-ready continuation；证据见 `docs/evidence/harness-continuation-audit-20260922/`）。**ADR-0085 P1（执行计划合同）已并入 `main`**：框架无关 `research-execution-plan.v1`、one-current-plan-per-task 持久化、task/plan version 原子 CAS、精确 approval/reference/idempotency/expected-postcondition、legacy ambiguity→needs_attention、内部 store seam 与只读 Product GET；P1 未暴露任何 execution-plan 写路由。**ADR-0085 P2（审批与异步事件统一）已并入 `main`**：框架无关 `research-continuation-event.v1` 封闭事件合同与确定性 reducer、五个服务端具名 adapter（精确 `agent_approvals`/`signal_producer_jobs`/`backtest_jobs`/耐久 trigger receipt identity，禁止按 latest 选行）、同一 task 最多一个 open continuation 的 PostgreSQL partial unique index、审批请求时铸造的精确 plan-command 绑定（task/workspace/plan/task version/action/resource/参数摘要/业务幂等键）、claim/settle 的耐久 pending intent，且不新增任何通用事件写 HTTP/MCP/Browser 路由；证据见 `docs/evidence/harness-continuation-audit-20260922/` 与 `services/backend/tests/test_research_continuation_*`。**ADR-0085 P3（有界研究判断回合）已并入 `main`**：框架无关有界 `research-stage-input.v1`/`research-proposal.v1` 封闭合同、有界 stage input（排除 raw/full signal snapshot、bars/frame/index 列表并设显式 byte/item 上限）、只读前台有界 `research-judgment-turn` 角色（静态精确只读 allowlist，模型无写/审批/路由/身份/幂等工具）、每 stage 耐久两次模型调用准入、原子 proposal+progress 提交与首次无 durable progress 即 `needs_attention/no_durable_progress` 栅栏；不新增任何通用 plan/event/proposal 写路由；证据见 `docs/evidence/adr-0085-p3-research-judgment/` 与 `services/backend/tests/test_research_judgment*`。P0–P4 共同组成 0.9.1 稳定性版本，不推进 Product Phase；0.9.1 正式发布/部署仍需独立授权。 | **下一个且唯一可执行任务 = ADR-0085 P4 真实闭环与故障矩阵**：使用真实 Product API + DSH 完成复合任务 → 对话内 continuation grant → 策略审批 → 数据就绪 → 回测执行审批 → 三轮完成/分析/修正 → 选择最优 → 模拟账户审批/创建 → task completed；每个异步交接点注入一次 Gateway/Backend/Adapter/Worker 重启，断言无重复对象、精确预算、任务/会话/generation/回执状态一致、模型不可见 raw/原始行情或完整执行快照。只允许隔离非生产 canary；生产部署、正式 canary、版本号写入、tag/release 仍需独立授权。0.10 与 Phase 100 恢复继续冻结。 | 维护者 2026-09-22 明确接受 ADR-0085 并授权 P0→P4 每步独立 PR | 不 deploy、不 tag/release、不恢复 Phase 100、不启动 0.10；不建第二通用 harness；不放宽逐动作审批；每步独立 worktree/PR；前一步合并后再开始下一步 |
...
- **构建身份同步**：`scripts/`/`tests/`/`services/runtime-adapter` 属 build inputs，构建身份推进
  `post-u8.200 → post-u8.201`（评估建立提交），一致性修复（顶部权威表 + DSH 计划措辞 + 治理测试）
  再推进 `post-u8.201 → post-u8.202`（`config/dsh/builds/dsh-0.1.5rc1-post-u8.202.json`；
  `scripts/dsh/build_revision.py`、`Dockerfile.post-u8-candidate`、
  `scripts/v090/dsh_default_upgrade/verify.py` 同步；`.200`/`.201` 与全部历史 manifest/证据保留）。
  ...
  `promoted_descriptor_hash` 同步为 `.202`/新 descriptor；`.200`/`.201` manifest 与历史 `changes`
  保持不变，未改任何结论。
...
本批执行 0.9 保留收口顺序中的最后一步：**独立 0.9 最终开发收口**（development closeout）。
独立 worktree/分支 `codex/v090-final-closeout`，基于动态 `origin/main`（`561fe92`，含 #355）。
这是维护/验收，**不推进 Product Phase**，**不部署生产**，不创建/移动 tag/release，不恢复
Phase 100，不启动 0.10，不自动解冻 R3，不做 Community 检查或复制，不实现产品新功能。

- **机器可读、可失败的最终矩阵**：`scripts/v090/final_closeout/{contract.v1.json,observer.py,
  build_provenance.py}` 与 `docs/evidence/v090-final-closeout/`（`assessment-input.v1.json`、
  `provenance.v1.json`、`verdict.v1.json`、`negative-controls.v1.json`、`interface-audit.v1.json`）。
  观察器**不信任 PASS 标签**：它从原始已合并证据独立派生每个必需项、校验 provenance sha256、
  现场重跑 fail-closed 全接口审计、并重新运行具名 D15 superseding 派生（含其 provenance 校验）。
  提交 verdict `format_valid=true`、`honest=true`、`complete=true`、`all_pass=true`、exit 0。
- **派生结论 `V090_DEVELOPMENT_CLOSEOUT_COMPLETE`**（全部 15 个必需项等于证据派生的期望真值）：
  closeout 审计 COMPLETE；F2 unknown-result reconciliation COVERED（其唯一 open 原因是 H4 台账
  非当前，现现场审计 `complete=true`）；剩余全接口审计 COMPLETE（`discovered=reviewed=verified=569`、
  `missing=stale=fake_pass=0`）；复合研究故障回归 **PASS_AS_SCOPED**（旅程 PASS、13 个 gating 行
  全 PASS、恰有合同固定的 4 个非 gating 行诚实 BLOCKED：`approval-revoked`/`timeout-terminal`/
  `data-ready-continuation`/`runtime-adapter-tool-boundary`；声称 all-pass 被拒绝）；ADR-0082/0083
  决定 RECORDED_ACCEPTED；ADR-0084 取代门禁 containment/business recovery PASS（真实隔离 9/9）；
  coherent DSH `0.1.5-rc.1` 仓库默认升级 PASS；具名 D15 superseding assessment ESTABLISHED；
  historical D15-G `NO_GO_PRESERVED`（不改写、未重跑）；B1 `subagent-child-crash` 与 B2
  `subagent-byq-adapter-restart` `BLOCKED_EXTERNAL`；B3/B4 候选/资格层 `PASS_CANDIDATE`；原生
  独立 child resume `NOT_IMPLEMENTED`；`R3_RESUME = NO`。
- **门禁作用域**：B1/B2 保持 `BLOCKED_EXTERNAL`，按 ADR-0084 §3 **仅限制原生独立 child resume**，
  不 gate 0.9 开发收口、不 gate 候选兼容/仓库默认晋升、不 gate bounded R3 范围。B1/B2 未被降级，
  原生独立 child 恢复未被声称实现；观察器拒绝把这些限制隐藏或改写成 PASS（`negative-controls.v1.json`
  共 29 项控制全部被拒，28 项 defect-targeting，已知良好 fixture 正常通过）。
- **边界约束（机器校验）**：仓库默认 `dsh-0.1.5rc1` + 回滚候选 `dsh-0.1.2rc1`；`production_deployment=none`；
  `release_or_tag_created=false`（仓库无 `byq-release.v1` manifest）；`phase_100_resumed=false`
  （`byq:phase-100-p100-c=paused-not-delivery` 保持）；`zero_ten_started=false`；`r3_resume=NO`；
  `b1_b2_downgraded=false`；`next_state=maintainer-testing-and-0.9x-window`；`build_revision` 同步。
  观察器对缺项、哈希不符、越权约束、STATUS marker 冲突一律 fail closed。
- **正式 0.9.0 发布门禁保持独立且未启动**：正式 release manifest（源码 SHA、trusted-main 全量 CI、
  全服务 digest、实际组合、迁移分类、SBOM、备份、回退、独立发布验收、attestation）仍为
  `OPEN_NOT_ATTEMPTED`，**不属于**开发收口范围；仓库默认 DSH `0.1.5-rc.1` **不等于**生产部署。
  部署、正式 tag/release、生产 selector 切换均为独立维护者决定，本批未执行也未声称。
...
- Product Phase 49-97 与相应 Accepted ADR/计划均已完成；**下一 Product Phase 尚未授权**。
  独立数据/资格轨道 Phase 98 已授权、99/101 完成、100 进行中；**0.9 开发收口已完成**，当前维护
  进入维护者测试与 0.9.x 小版本功能追加/优化窗口；完整权威状态见顶部“权威当前状态”。
- 2026-09-22 **0.9 独立最终开发收口**已完成：机器可读、可失败的最终矩阵
  [`docs/evidence/v090-final-closeout/`](../evidence/v090-final-closeout/README.md) 派生
  `V090_DEVELOPMENT_CLOSEOUT_COMPLETE`；B1/B2 保持 `BLOCKED_EXTERNAL`（仅限制原生独立 child
  resume），历史 D15-G 仍 `NO_GO`（不改写、未重跑），`R3_RESUME = NO`，仓库默认 DSH
  `0.1.5-rc.1` **不等于**生产部署。**0.10 与 Phase 100 恢复仍为未授权硬停止**；本批不 deploy、
  不创建/移动 tag/release、不启动 R3。
