...
def build(profile_name: str | None = None) -> tuple[str, dict[str, Any]]:
    ...
    for plugin_id in enabled_ids:
        ...
        for agent_id in plugin["agents"]["allowed"]:
            if plugin_id in data["agents"]["agents"][agent_id]["plugins"]:
    ...
    identity_input = {
        ...
        "agent_assignments": data["agents"]["agents"],
    }
    ...
    header = (
        ...
        f"# plugin-profile: {data['profile_name']}\n"
        f"# composition-hash: sha256:{digest}\n"
    )
    return header + body, identity


def write_or_check(*, profile_name: str | None, check: bool) -> None:
    composition, identity = build(profile_name)
...
def main() -> int:
    ...
    try:
        if args.command == "build":
            write_or_check(profile_name=args.profile, check=args.check)
