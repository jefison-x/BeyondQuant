#!/usr/bin/env python3
"""ADR-0085 P4 keyless judgment-aware scripted provider (isolated non-production).

A non-LLM OpenAI-compatible streaming provider used ONLY by the isolated P4
four-boundary stack. It drives the REAL DSH child/tool path:

  root request (tools include byq_research_judgment_turn)
    -> emit one tool call to the bounded persona, carrying the bounded stage input
  child request (tools are the five read-only MCP tools, no persona tool)
    -> emit the closed `{"proposal": <research-proposal.v1>}` JSON as the child's
       final message, using the authoritative task_id/plan_version/task_version/
       stage/iteration copied from the stage input
  root request after the tool result
    -> emit a short final text and stop

It never calls a paid/provider API and holds no credential. The evidence MUST be
labelled scripted-keyless / non-real-LLM-quality.

Control endpoints (evidence only):
  GET /_calls  -> {"calls":N,"root_calls":N,"child_calls":N,"tool_calls_emitted":N,...}
  GET /healthz
"""
...
def _summarize_stage_input(stage_input: object) -> dict:
    """Safe metrics of the ACTUAL model-visible bounded input (never the raw text)."""
...
def _proposal_from_stage_input(stage_input: dict | None) -> dict:
    ...
    The isolated scripted turn exercises the normal bounded outcome: a
    well-formed proposal for the current stage/version whose committed plan
    references (bound deterministically from the user-confirmed validated
    artifacts) let the Backend mint the strategy_version-bound approval. The
    model supplies no object identity, approval or routing.
    """
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if urlparse(self.path).path in {"/_calls", "/healthz"}:
            self._json({
                "service": "byq-p4-judgment-provider", "calls": self.calls,
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if is_root and not has_tool_result:
            ...
            call_id = f"p4-judgment-{self.tool_calls_emitted}"
            arguments = {
...
def main() -> int:
    ...
    print(f"byq-p4-judgment-provider listening on {port}", flush=True)
